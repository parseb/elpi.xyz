...
// DELETE /api/quotes/[hash] — soft-delete (mark historical) or hard-delete (?hard=true) a quote by its hash.
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ hash: string }> }) {
  ...
  const result = hard
    ? db.store.deleteQuote(hash)
    : db.store.invalidateQuote(hash);
...
// PATCH /api/quotes/[hash] — update status (active vs historical)
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ hash: string }> }) {
  ...
  try {
    ...
    const result = db.store.setQuoteStatus(hash, status);
    if (result.changes === 0) {
      return NextResponse.json({ error: "Quote not found" }, { status: 404 });
    }
